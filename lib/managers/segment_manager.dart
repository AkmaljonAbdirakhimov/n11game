...
import '../objects/objects.dart';
 ...
 List<List<ObjectBlock>> generateRandomSegments(int count) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      for (int x = 0; x < 10; x++) {
        if (random.nextDouble() > 0.1) { // 10% chance to create a gap
          segment.add(ObjectBlock(blockType: GroundBlock, gridPosition: Vector2(x.toDouble(), 0)));
          occupied[x][0] = true;
        }
      }
      ...
      for (int x = 0; x < 10; x++) {
        if (random.nextDouble() < 0.2) {  // 20% chance to start a platform group
          ...
          for (int j = 0; j < platformLength; j++) {
            if (x + j < 10 && !occupied[x + j][y]) {
              segment.add(ObjectBlock(blockType: PlatformBlock, gridPosition: Vector2((x + j).toDouble(), y.toDouble())));
              occupied[x + j][y] = true;
            }
          }
      ...
      // Randomly add stars
      for (int x = 0; x < 10; x++) {
        if (random.nextDouble() < 0.15) {  // 15% chance to add a star
          int y = random.nextInt(4) + 1;  // Star height between 1 and 4
          if (!occupied[x][y] && (occupied[x][y - 1] || y == 1)) {
            segment.add(ObjectBlock(blockType: Star, gridPosition: Vector2(x.toDouble(), y.toDouble())));
            occupied[x][y] = true;
          }
        }
      }
      ...
      while (waterEnemiesCount < 1) {  // Ensure only 1 water enemy per segment
        int x = random.nextInt(10);
        for (int y = 1; y <= 4; y++) {  // Search for a ground or platform to place the water enemy
          if (occupied[x][y - 1] && !occupied[x][y]) {  // Place water enemy on a ground or platform
            segment.add(ObjectBlock(blockType: WaterEnemy, gridPosition: Vector2(x.toDouble(), y.toDouble())));
            occupied[x][y] = true;
            waterEnemiesCount++;
            break;
          }
